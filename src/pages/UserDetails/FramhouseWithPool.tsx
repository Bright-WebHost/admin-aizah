...
const ROOM_NAME = "FramhouseWithPool";

const FramhouseWithPool: React.FC = () => {
  ...
  const fetchBookedDates = useCallback(async () => {
    try {
      ...
      if (data.data && Array.isArray(data.data)) {
        const meranoBookings = data.data.filter((booking: any) => 
          booking.roomname === ROOM_NAME
        );
        
        const formattedDates = meranoBookings.map((booking: any) => ({
          _id: booking._id,
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const meranoEvents = eventsArray.filter((event: any) => 
          event.roomname === ROOM_NAME
        );

        const mappedEvents: CalendarEvent[] = meranoEvents.map((event: any) => ({
          ...
          extendedProps: {
            ...
            roomname: event.roomname || ROOM_NAME
          },
        }));
    ...
    <>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-xl font-semibold dark:text-white">Framhouse WithPool </h2>
          {/* <button
            onClick={togglePostbookFilter}
            className={`px-4 py-2 rounded-lg ${showOnlyPostbook ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          >
            {showOnlyPostbook ? 'Show All' : 'Show Only Offline Payments'}
          </button> */}
        </div>
          ...
          <div className="flex flex-col px-2 overflow-y-auto custom-scrollbar">
            ...
            <div className="mt-6">
              ...
              <textarea
                ...
                className="h-24 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
